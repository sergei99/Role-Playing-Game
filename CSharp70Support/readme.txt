Current C# version - 7.2

### How to install ###

1. Copy this CSharp70Support folder to your Unity project. It should be placed
   in the project's root, next to the Assets folder (not inside it!).

2. Import CSharp70Support/CSharpVNextSupport.unitypackage into your Unity project.

3. [Optional] On Windows, run CSharp70Support/ngen install.cmd once with administrator
   privileges. It will precompile csc.exe, pdb2mdb.exe using Ngen, that will make compilation
   process in Unity a bit faster.

### History ###

CSharpVNextSupport 4.0.1

    - Added missing reference to System.Xml.Linq.dll

CSharpVNextSupport 4.0.0

    - Unity 2018.1 support
    
    - The integration mechanism was changed. These's no longer need to mess with files in Unity installation folder.
      UnityEditor.dll is patched in memory using Harmony (https://github.com/pardeike/Harmony).